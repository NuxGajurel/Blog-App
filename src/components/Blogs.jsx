...
const Blogs = () => {
  ...
  const[search ,setSearch]=useState("");
    const filteredBlogs = infoBlogs.filter(infoBlog =>
...
<input type="text" value={search} placeholder='Search Blogs.....' onChange={(e) => setSearch(e.target.value)} className='mt-33 border border-gray-300 rounded-lg px-4 py-2 w-full max-w-md focus:outline-none m-4 focus:ring focus:ring-blue-300'/>
   ...
   {filteredBlogs.length > 0 ?(
          ...
          filteredBlogs.map((infoBlog)=>(
            <Link key={infoBlog.id} to={infoBlog.link} className="overflow-hidden rounded shadow-lg max-w-sm">
          <img src={infoBlog.image} alt="Blog 1" className="w-full"/>
          <div>
          <h2 className='text-2xl m-3 text-black'>{infoBlog.title} </h2>
          <p className='m-3 text-black'>-{infoBlog.author}</p> <i className='flex justify-end m-3 text-2xl text-black'> <FaArrowRightLong/></i>
          </div>
        </Link>
          ))
      ...
      (
       <p className="text-center text-gray-500 col-span-full">No Bolgs Found</p>
      )
     }
